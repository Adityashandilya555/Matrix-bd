...
from app.services.legal_service import (
    agreement_to_response,
    dd_to_response,
    fetch_agreement_or_none,
    fetch_dd_or_none,
    fetch_licensing_or_none,
    licensing_to_response,
)
...
async def build_tracker_response(
    ...
    *,
    ...
    dd  = await fetch_dd_or_none(db, site_id=site.id)
    ag  = await fetch_agreement_or_none(db, site_id=site.id)
    lic = await fetch_licensing_or_none(db, site_id=site.id)

    dd_resp        = dd_to_response(dd)         if _visible(dd)  else None
    agreement_resp = agreement_to_response(ag)  if _visible(ag)  else None
    licensing_resp = licensing_to_response(lic) if _visible(lic) else None
