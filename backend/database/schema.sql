...
CREATE TABLE IF NOT EXISTS public.module_codes (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  tenant_id uuid NOT NULL,
  module text NOT NULL CHECK (module IN ('bd','legal','payment','design','project','nso')),
...
CREATE TABLE IF NOT EXISTS public.nso_reviews (
  site_id uuid NOT NULL,
  tenant_id uuid NOT NULL,
  current_stage text NOT NULL DEFAULT 'stage_one' CHECK (current_stage IN ('stage_one','stage_two','stage_three','final','done')),
  nso_status text NOT NULL DEFAULT 'pending' CHECK (nso_status IN ('pending','in_progress','complete')),
  ...
  dry_stock_order_status text NOT NULL DEFAULT 'pending' CHECK (dry_stock_order_status IN ('pending','ordered','received')),
  online_delivery_status text NOT NULL DEFAULT 'pending' CHECK (online_delivery_status IN ('pending','ready','active')),
...
CREATE INDEX IF NOT EXISTS idx_nso_reviews_tenant_status ON public.nso_reviews(tenant_id, nso_status);
