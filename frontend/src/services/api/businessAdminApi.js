...
export async function getFinanceQueue() {
  const d = await client.get('/business-admin/finance-queue').then((r) => r.data);
  return {
    items: (d.items || []).map((r) => ({
...
export async function getAllSites() {
  const d = await client.get('/sites').then((r) => r.data);
  return {
    items: (d.items || []).map((s) => ({
      siteId: s.id, siteCode: s.code || '', siteName: s.name, city: s.city, status: s.status,
      createdByName: s.created_by, assignedToName: s.assigned_to_name, visitDate: s.visit_date, days: s.days,
...
export async function getSiteHistory(siteId) {
  const d = await client.get(`/sites/${siteId}/activity`).then((r) => r.data);
  return {
    items: (d.items || []).map((e) => ({
