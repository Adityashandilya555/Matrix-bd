...
export default function AddDetailsPage({ item, onClose, onSubmit, onSaveDraft }) {
  ...
  const totalFields = REQUIRED.length + 1;
  const filledCount = REQUIRED.filter(k => f[k] || f[k] === 0).length + (f.photos.length > 0 ? 1 : 0);
      ...
      <div style={{ background: 'var(--zm-bg)', borderLeft: '1px solid var(--zm-line)', width: 880, maxWidth: '96%', display: 'flex', flexDirection: 'column', boxShadow: 'var(--zm-shadow-pop)', animation: 'zm-slide 280ms var(--zm-ease-emp)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px 28px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <FormSection n="8·11" title="Score + adjacency sales"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}><TextField label="Score" value={f.score} onChange={upd('score')} required type="number" min="0" hint="0–100 footfall + visibility"/><TextField label="Estimated sales" value={f.estSales} onChange={upd('estSales')} required mono prefix="₹" suffix="/mo"/><TextField label="Nearest Starbucks sales" value={f.nearestStarbucks} onChange={upd('nearestStarbucks')} required mono prefix="₹" suffix="/mo"/><TextField label="Nearest TWC sales" value={f.nearestTWC} onChange={upd('nearestTWC')} required mono prefix="₹" suffix="/mo" hint="Third-Wave Coffee"/></div></FormSection>
            <FormSection n="12·14" title="Carpet · CAM · rent">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}><TextField label="Carpet / covered area" value={f.carpet} onChange={upd('carpet')} required mono suffix="sqft"/><TextField label="CAM" value={f.cam} onChange={upd('cam')} required mono prefix="₹" suffix="/mo"/><div/></div>
              <div style={{ background: 'var(--zm-surface)', border: '1px solid var(--zm-line)', borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
                  {f.rentType === 'fixed' && (<><TextField label="Rent (monthly)" value={f.rent} onChange={upd('rent')} required mono prefix="₹" suffix="/mo"/><TextField label="Escalation" value={f.escalation} onChange={upd('escalation')} required mono suffix="% / yr"/><TextField label="Rent-free days" value={f.rentFreeDays} onChange={upd('rentFreeDays')} mono suffix="days" hint="Optional fit-out grace"/></>)}
                  {f.rentType === 'revshare' && (<><TextField label="Revenue share" value={f.revshare} onChange={upd('revshare')} required mono suffix="% of sales" error={errors.revshare}/><TextField label="Min guarantee" value={f.rent} onChange={upd('rent')} mono prefix="₹" suffix="/mo" hint="If applicable" required/><TextField label="Rent-free days" value={f.rentFreeDays} onChange={upd('rentFreeDays')} mono suffix="days" hint="Optional"/></>)}
            ...
            <FormSection n="15·17" title="Capex · deposit · brokerage"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}><TextField label="Cadex (capex)" value={f.cadex} onChange={upd('cadex')} required mono prefix="₹" hint="Fit-out budget"/><TextField label="Security deposit" value={f.deposit} onChange={upd('deposit')} required mono prefix="₹"/><TextField label="Brokerage" value={f.brokerage} onChange={upd('brokerage')} required mono prefix="₹"/></div></FormSection>
