...
function ExecRow({ site, onUpload, onOpen, uploading }) {
  const remaining = site.expectedLoiDays - site.daysSinceApproval;
  const overdue = remaining < 0;
  const uploaded = site.loiUploaded;
  // A supervisor sent the LOI back: the site returned to APPROVED and carries
  // the reason. The upload control comes back so the correct file can go up.
  const sentBack = !uploaded && !!site.loiRejectionNote;
  ...
  return (
    <div className="zm-row" data-site-id={site.id} style={{ display: 'grid', gridTemplateColumns: '0.9fr 1.6fr 1fr 1fr 1fr 1.4fr 170px', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: '1px solid var(--zm-line-faint)', background: overdue && !uploaded ? 'rgba(217,119,6,0.06)' : 'transparent', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}><span style={{ fontFamily: 'var(--zm-font-body)', fontSize: 13.5, fontWeight: 600, color: 'var(--zm-fg)' }}>{site.name}</span><span style={{ fontFamily: 'var(--zm-font-mono)', fontSize: 10.5, color: 'var(--zm-fg-3)' }}>by {site.createdBy}</span>{sentBack && (<span title={site.loiRejectionNote} style={{ marginTop: 2, fontFamily: 'var(--zm-font-body)', fontSize: 10.5, fontWeight: 600, color: 'var(--zm-warning)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Sent back: {site.loiRejectionNote}</span>)}</div>
      ...
      <div>{uploaded ? <StatusPill stage="uploaded"/> : sentBack ? <StatusPill stage="overdue"/> : overdue ? <StatusPill stage="overdue"/> : <StatusPill stage="staging"/>}</div>
      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
        ...
        {uploaded ? (<button disabled style={{ height: 32, padding: '0 12px', border: '1px solid var(--zm-line)', borderRadius: 7, background: 'var(--zm-surface)', color: 'var(--zm-fg-3)', fontFamily: 'var(--zm-font-body)', fontSize: 12, fontWeight: 600, cursor: 'not-allowed', display: 'inline-flex', alignItems: 'center', gap: 6 }}><Icon name="check" size={12}/> Uploaded</button>) : (<><input ref={fileRef} type="file" accept=".pdf,.doc,.docx" style={{ display: 'none' }} onChange={handleFile}/><button onClick={() => fileRef.current?.click()} disabled={uploading} aria-busy={uploading} className="zm-btn-primary" style={{ height: 32, padding: '0 12px', border: 'none', borderRadius: 7, background: overdue ? 'var(--zm-warning)' : 'var(--zm-accent)', color: 'var(--zm-accent-on)', fontFamily: 'var(--zm-font-body)', fontSize: 12, fontWeight: 700, cursor: uploading ? 'wait' : 'pointer', opacity: uploading ? 0.65 : 1, display: 'inline-flex', alignItems: 'center', gap: 6, boxShadow: 'var(--zm-shadow-1)' }}>{uploading ? 'Uploading…' : <><Icon name="upload" size={12}/> {sentBack ? 'Re-upload LOI' : 'Upload LOI'}</>}</button></>)}
      </div>
    </div>
  );
}
...
export default function ExecStagingPage({ onOpenSite: onOpenSiteProp, showToast: showToastProp }) {
  ...
  // Rows with an upload in flight, so each one's button shows a busy state and
  // can't be triggered again (a second file-pick mid-upload would fire a
  // duplicate uploadLOI). The ref guards the same-tick double-fire a disabled
  // attribute alone can't (state hasn't re-rendered yet); the state Set drives
  // the per-row busy rendering. Mirrors the Push button fix (#414).
  const [uploadingIds, setUploadingIds] = React.useState(() => new Set());
  const uploadingRef = React.useRef(new Set());

  const onUpload = async (site, file) => {
    if (uploadingRef.current.has(site.id)) return;
    uploadingRef.current.add(site.id);
    setUploadingIds((prev) => new Set(prev).add(site.id));
    try {
      ...
      uploadingRef.current.delete(site.id);
      setUploadingIds((prev) => { const next = new Set(prev); next.delete(site.id); return next; });
    }
  };
