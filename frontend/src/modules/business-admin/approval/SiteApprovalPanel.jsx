...
const taStyle = {
  width: '100%', boxSizing: 'border-box', minHeight: 52, padding: 9, borderRadius: T.radiusSm,
  border: `1px solid ${T.lineStrong}`, background: T.surfaceInset, color: T.text, fontSize: 12.5,
  resize: 'vertical', marginBottom: 9, fontFamily: 'inherit', lineHeight: 1.5,
};
...
function GfcBlock({ siteId, fetchReview, onDecide }) {
  ...
  if (!detail) return <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}><Skeleton h={15} w="60%" /><Skeleton h={15} w="48%" /></div>;
  return (
    <>
      <div style={{ display: 'grid', gap: 9, marginBottom: 12 }}>
        {(detail.deliverables || []).map((d) => (
          <div key={d.kind} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12.5, flexWrap: 'wrap' }}>
            <span style={{ width: 104, color: T.textMuted, flexShrink: 0 }}>{KIND_LABEL[d.kind] || d.kind}</span>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <Button variant="success" size="md" loading={busy} icon={!busy && <Icon.check size={15} />}
          onClick={() => decide('approve')}>Approve · Good-For-Construction</Button>
