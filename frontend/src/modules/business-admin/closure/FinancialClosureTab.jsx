...
//
// Backed by GET /financial-closure/admin-queue, not the /queue the BD-side tab
// uses: that one scopes an executive to their own allocations, which is wrong
// for an admin who should see the whole tenant. Both return the same row shape,
// so the status vocabularies and the pending-with rule are shared with the BD
// tab through ../../financial_closure/closureStatus.js.
//
...
import { getFCAdminQueue, getFCAdminDetail } from '../../../services/api/financialClosureApi.js';
...
export default function FinancialClosureTab() {
  ...
  const pending = queue.items.filter((r) => PENDING_STATUSES.includes(r.financialClosureStatus));
  const closed = queue.items.filter(isClosed);
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {queue.status === 'ready' && rows.map((row) => (
