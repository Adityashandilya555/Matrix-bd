...
import { statusMeta, EyeIcon, ReasonDialog } from '../../archive/ArchivePage.jsx';
  ...
  return (
    <div style={{ background: 'var(--zm-surface)', border: '1px solid var(--zm-line)', borderRadius: 10, padding: 12, display: 'flex', flexDirection: 'column', gap: 8, minWidth: 248 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
        ...
        {cells.map((d, i) => { const cellKey = d ? `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}` : `null-${i}`; if (!d) return <span key={cellKey} style={{ height: 28 }}/>; const startSel = sameDay(d, fromD); const endSel = sameDay(d, toD); const within = inRange(d) && !startSel && !endSel; return (<button key={cellKey} onClick={() => pick(d)} className="zm-cal-day" data-state={startSel ? 'start' : endSel ? 'end' : within ? 'within' : 'idle'} style={{ height: 28, padding: 0, border: 'none', borderRadius: startSel ? '999px 0 0 999px' : endSel ? '0 999px 999px 0' : within ? 0 : 6, background: (startSel || endSel) ? 'var(--zm-accent)' : within ? 'var(--zm-accent-soft)' : 'transparent', color: (startSel || endSel) ? 'var(--zm-accent-on)' : within ? 'var(--zm-accent)' : 'var(--zm-fg)', fontFamily: 'var(--zm-font-mono)', fontFeatureSettings: "'tnum' 1", fontSize: 12, fontWeight: (startSel || endSel) ? 700 : 500, cursor: 'pointer' }}>{d.getDate()}</button>); })}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 30, background: 'var(--zm-surface)', border: '1px solid var(--zm-line)', borderRadius: 12, boxShadow: 'var(--zm-shadow-pop)', width: 560, padding: 16, display: 'flex', flexDirection: 'column', gap: 16, animation: 'zm-rise 200ms var(--zm-ease-emp)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 8, background: 'var(--zm-bg-2)' }}>
        ...
        <span style={{ flex: 1 }}/>
        <button onClick={onClose} style={{ height: 30, padding: '0 14px', borderRadius: 7, border: 'none', background: 'var(--zm-accent)', color: 'var(--zm-accent-on)', fontFamily: 'var(--zm-font-body)', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>Apply</button>
      </div>
    </div>
  );
}
...
// ArchiveTable — same row shape as the Archived / rejected page, minus revive
// (that action lives on the dedicated page).
const ARCH_COLS = '0.9fr 1.6fr 1fr 1fr 1fr 1.4fr 90px';
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--zm-surface)', border: '1px solid var(--zm-line)', borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--zm-shadow-1)' }}>
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {rows.map(a => {
          ...
          return (
            <div key={a.id} className="zm-row" style={{ display: 'grid', gridTemplateColumns: ARCH_COLS, gap: 10, padding: '12px 16px', borderBottom: '1px solid var(--zm-line-faint)', alignItems: 'flex-start' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 152px)', minHeight: 400 }}>
      ...
      {view === 'archived' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <ArchiveTable rows={filteredArchive} onOpen={(a) => onOpenSite?.(a)} onViewReason={setViewingReason}/>
